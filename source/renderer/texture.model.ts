export class Texture {
  ...
  private static createTextureTemplate(width: number, height: number) {
  ...
  public static fromRaw(
    raw: ArrayBuffer, 
    mips: Array<ArrayBuffer> = [],
    size: { width: number, height: number } = { height: 0, width: 0 }
  ) {

    const texture = Texture.createTextureTemplate(size.width, size.height);

    queueMicrotask(() => {

      const row = Uint8Array.BYTES_PER_ELEMENT * 4 * size.width;

      device.queue.writeTexture({ texture }, raw, {
        bytesPerRow: row,
      }, size);

      for ( let i = 0; i < mips.length; i++ ) {
        device.queue.writeTexture({ texture }, mips[i], {
          bytesPerRow: row >> (1 + i)
        }, size);
      }
  ...
  public static async fromBuffer(buffer: ArrayBuffer, mips: Array<ArrayBuffer> = []): Promise<Result<Texture>> {
    ...
    if ( import.meta.env.DEV && bitmap.width < 256 || bitmap.height < 256 ) return Error("Texture should be larger than 256x256 px")
  ...
  private static create(
    ...
    const texture = Texture.createTextureTemplate(width, height);
